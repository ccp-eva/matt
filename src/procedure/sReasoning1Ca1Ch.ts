...
export default async ({ currentSlide, previousSlide }) => {
	...
	const slidePrefix = 'sr1cat1chicken';
	...
	// check actual responses from s1Ca1Ch and overwrite default values
	if (data.procedure.s1Ca1Ch) {
		wasCat = data.procedure.s1Ca1Ch.response.toLowerCase().includes(`-one${leftEntity}`);
		wasCantDecide = data.procedure.s1Ca1Ch.response.toLowerCase().includes('-cantdecide');
		wasChicken = data.procedure.s1Ca1Ch.response.toLowerCase().includes(`-one${rightEntity}`);
		// check if order was swapped, if so swap boxes
		if (data.procedure.s1Ca1Ch.swapLeftRight) {
			const left = document.getElementById(`${slidePrefix}-${leftEntityOne}`)! as SvgInHtml;
			const right = document.getElementById(`${slidePrefix}-${rightEntityOne}`)! as SvgInHtml;
	...
	await gsap
		.timeline()
		.to('#s-reasoning-1-ca-1-ch', { autoAlpha: 0 })
