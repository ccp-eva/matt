...
export default async ({ currentSlide, previousSlide }) => {
	...
	var circles_id = ['st-inner1', 'st-middle1', 'st-outer1'];
	...
	// always show human
	knownAnimals = [...knownAnimals, 'humanidj'];
	...
	// store data
	data.procedure.sTaskIdj.knownAnimals = knownAnimals;
	data.procedure.sTaskIdj.unknownAnimals = unknownAnimals;
	data.procedure.sTaskIdj.knownAnimalOrder = knownAnimalOrder;
	...
	const dragObjects = Draggable.create(knownAnimalElements, {
		...
		onDragEnd: function () {
			...
			if (circleDistance * 1.2 <= innerRadius + targetWidth) {
				...
				data.procedure.sTaskIdj[currentIdTrimmed].circle = 'inner';
				data.procedure.sTaskIdj[currentIdTrimmed].coords.x = droppedX;
				data.procedure.sTaskIdj[currentIdTrimmed].coords.y = droppedY;
			...
			if (
				...
				data.procedure.sTaskIdj[currentIdTrimmed].circle = 'middle';
				data.procedure.sTaskIdj[currentIdTrimmed].coords.x = droppedX;
				data.procedure.sTaskIdj[currentIdTrimmed].coords.y = droppedY;
			...
			if (
				...
				data.procedure.sTaskIdj[currentIdTrimmed].circle = 'outer';
				data.procedure.sTaskIdj[currentIdTrimmed].coords.x = droppedX;
				data.procedure.sTaskIdj[currentIdTrimmed].coords.y = droppedY;
			...
			// NONE
			if (circleDistance * 1.2 > outerRadius + targetWidth) {
				if (data.procedure.sTaskIdj[currentIdTrimmed].circle === undefined) {
					...
					gsap.to(currentTarget, {
						x: data.procedure.sTaskIdj[currentIdTrimmed].coords.x,
						y: data.procedure.sTaskIdj[currentIdTrimmed].coords.y,
						scale: 0.5,
					});
				}
			}
