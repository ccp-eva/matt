...
export default async ({ currentSlide, previousSlide }) => {
	...
	const slidePrefix = 'sr1zdog1zchicken';
	...
	// check actual responses from s1Zd1Zc and overwrite default values
	if (data.procedure.s1Zd1Zc) {
		wasDog = data.procedure.s1Zd1Zc.response.toLowerCase().includes(`-one${leftEntity}`);
		wasCantDecide = data.procedure.s1Zd1Zc.response.toLowerCase().includes('-cantdecide');
		wasChicken = data.procedure.s1Zd1Zc.response.toLowerCase().includes(`-one${rightEntity}`);
		// check if order was swapped, if so swap boxes
		if (data.procedure.s1Zd1Zc.swapLeftRight) {
			const left = document.getElementById(`${slidePrefix}-${leftEntityOne}`)! as SvgInHtml;
			const right = document.getElementById(`${slidePrefix}-${rightEntityOne}`)! as SvgInHtml;
	...
	await gsap
		.timeline()
		.to('#s-reasoning-1-zd-1-zc', { autoAlpha: 0 })
