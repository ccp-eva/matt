...
export default async ({ currentSlide, previousSlide }) => {
	...
	const slidePrefix = 'sr1dog1duiker';
	const leftEntity = 'dog';
	const leftEntityOne = 'oneDog';
	const rightEntity = 'duiker';
	const rightEntityOne = 'oneDuiker';
	...
	let wasDuiker = false;
	// check actual responses from s1Do1Du and overwrite default values
	if (data.procedure.s1Do1Du) {
		wasDog = data.procedure.s1Do1Du.response.toLowerCase().includes(`-one${leftEntity}`);
		wasCantDecide = data.procedure.s1Do1Du.response.toLowerCase().includes('-cantdecide');
		wasDuiker = data.procedure.s1Do1Du.response.toLowerCase().includes(`-one${rightEntity}`);
		// check if order was swapped, if so swap boxes
		if (data.procedure.s1Do1Du.swapLeftRight) {
			const left = document.getElementById(`${slidePrefix}-${leftEntityOne}`)! as SvgInHtml;
			const right = document.getElementById(`${slidePrefix}-${rightEntityOne}`)! as SvgInHtml;
	...
	await gsap
		.timeline()
		.to('#s-reasoning-1-do-1-du', { autoAlpha: 0 })
