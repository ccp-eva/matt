...
export default async ({ currentSlide, previousSlide }) => {
	...
	data.procedure.sTaskZm = {
		duration: 0,
		knownAnimals: [],
		unknownAnimals: [],
		humanzm: { circle: undefined, coords: { x: 0, y: 0 } },
	...
	const nextButton = document.getElementById('link-stzm-next')! as SvgInHtml;
	const pinda = document.getElementById('pinda') as HTMLVideoElement;
	const audio = document.getElementById('audio') as HTMLMediaElement;
	const headphones = document.getElementById('link-stzm-headphones') as SvgInHtml;
	const circle = document.getElementById('stzm-circle')! as SvgInHtml;
	var circles_id = [
		'st-inner_00000132051808926935315550000005190983711798028972_',
		'st-middle_00000173871758896415333820000011256690720601550766_',
		'st-outer_00000118380426042676283530000003752574715951801493_',
	];
	...
	// always show human
	knownAnimals = [...knownAnimals, 'humanzm'];
	...
	// store data
	data.procedure.sTaskZm.knownAnimals = knownAnimals;
	data.procedure.sTaskZm.unknownAnimals = unknownAnimals;
	data.procedure.sTaskZm.knownAnimalOrder = knownAnimalOrder;

	// use the ordered version
	const knownAnimalElements = knownAnimalOrder.map(
		(animal) => document.getElementById(`link-stzm-${animal}`)! as SvgInHtml
	);
	...
	const dragObjects = Draggable.create(knownAnimalElements, {
		...
		onDragEnd: function () {
			...
			if (circleDistance * 1.2 <= innerRadius + targetWidth) {
				...
				data.procedure.sTaskZm[currentIdTrimmed].circle = 'inner';
				data.procedure.sTaskZm[currentIdTrimmed].coords.x = droppedX;
				data.procedure.sTaskZm[currentIdTrimmed].coords.y = droppedY;
			...
			if (
				...
				data.procedure.sTaskZm[currentIdTrimmed].circle = 'middle';
				data.procedure.sTaskZm[currentIdTrimmed].coords.x = droppedX;
				data.procedure.sTaskZm[currentIdTrimmed].coords.y = droppedY;
			...
			if (
				...
				data.procedure.sTaskZm[currentIdTrimmed].circle = 'outer';
				data.procedure.sTaskZm[currentIdTrimmed].coords.x = droppedX;
				data.procedure.sTaskZm[currentIdTrimmed].coords.y = droppedY;
			...
			// NONE
			if (circleDistance * 1.2 > outerRadius + targetWidth) {
				if (data.procedure.sTaskZm[currentIdTrimmed].circle === undefined) {
					...
					gsap.to(currentTarget, {
						x: data.procedure.sTaskZm[currentIdTrimmed].coords.x,
						y: data.procedure.sTaskZm[currentIdTrimmed].coords.y,
						scale: 0.5,
					});
				}
			}
	...
	const checkAnimals = () => {
		...
		knownAnimals.forEach((animal) => {
			if (!data.procedure.sTaskZm[animal].circle) {
	...
	for (const [index, order] of data.procedure.sTaskZm.comprehension.order.entries()) {
		...
		if (response.id.split('_')[0].split('-')[1] === order) {
			data.procedure.sTaskZm.comprehension[order] = true;
		}
	}
	console.log(data.procedure.sTaskZm.comprehension);

	data.procedure.sTaskZm.comprehension.completed = true;
